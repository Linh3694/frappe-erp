...
# -*- coding: utf-8 -*-
"""
Script kiểm tra cấu hình comment môn Toán trong báo cáo học tập.
Chạy trong bench console: bench --site <site> console
Sau đó: exec(open('scripts/check_math_comment_config.py').read())

Hoặc chạy trực tiếp: bench --site <site> execute erp.scripts.check_math_comment_config.run_check
"""
...
def run_check(
    ...
    Args:
        template_name: Tên template (vd: jt7dm67d31)
        form_id: Form ID (vd: ipa8vd9f9c)
        subject_id: Mã môn Toán (SIS_ACTUAL_SUBJECT-01332)
        comment_title_id: Mã tiêu đề nhận xét (khen---cần-khắc-phục---giải-pháp-CAMPUS-00001)
    """
    print("=" * 70)
    print("KIỂM TRA CẤU HÌNH COMMENT MÔN TOÁN - BÁO CÁO HỌC TẬP")
    print("=" * 70)

    # 1. Tìm template theo name hoặc form_id
    template = None
    if template_name:
        try:
    ...
    # 2. Query subject config từ DB (trực tiếp từ tab child)
    ...
    # Parse comment_title_options từ DB
    raw_opts = row.get("comment_title_options")
    if raw_opts:
        try:
            opts = json.loads(raw_opts) if isinstance(raw_opts, str) else raw_opts
            print(f"  comment_title_options (từ DB - snapshot): {json.dumps(opts, ensure_ascii=False, indent=4)}")
        except Exception as e:
            print(f"  comment_title_options (raw): {raw_opts}")
            print(f"  Lỗi parse JSON: {e}")
    else:
        print(f"  comment_title_options: NULL/empty (không có snapshot trong DB)")

    # 3. Load options từ SIS Report Card Comment Title (nguồn master)
    print(f"\n--- OPTIONS TỪ SIS REPORT CARD COMMENT TITLE ({comment_title_id}) ---")
    try:
        ...
        if hasattr(comment_doc, "options") and comment_doc.options:
            ...
            print(f"  options (từ Comment Title master): {json.dumps(opts_from_master, ensure_ascii=False, indent=4)}")
        else:
            print("  options: (trống)")
    except frappe.DoesNotExistError:
    ...
    print("\n--- KẾT LUẬN ---")
    if not raw_opts:
        print("  • comment_title_options trong DB là NULL → Khi API serialize, sẽ trả về None.")
        ...
        print("  • Nếu khác với Comment Title master → có thể do lần save trước frontend đã gửi snapshot khác.")
