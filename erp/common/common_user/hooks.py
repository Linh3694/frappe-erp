...
def setup_user_management_hooks():
    ...
    try:
        ...
        # Add our hooks
        if "erp.user_management.hooks.create_user_profile_on_user_creation" not in frappe.get_hooks("doc_events")["User"]["after_insert"]:
            frappe.get_hooks("doc_events")["User"]["after_insert"].append("erp.user_management.hooks.create_user_profile_on_user_creation")
        
        if "erp.user_management.hooks.update_user_profile_on_user_update" not in frappe.get_hooks("doc_events")["User"]["on_update"]:
            frappe.get_hooks("doc_events")["User"]["on_update"].append("erp.user_management.hooks.update_user_profile_on_user_update")
        
        if "erp.user_management.hooks.delete_user_profile_on_user_deletion" not in frappe.get_hooks("doc_events")["User"]["before_delete"]:
            frappe.get_hooks("doc_events")["User"]["before_delete"].append("erp.user_management.hooks.delete_user_profile_on_user_deletion")
        
        if "erp.user_management.hooks.validate_user_permissions" not in frappe.get_hooks("doc_events")["User"]["validate"]:
            frappe.get_hooks("doc_events")["User"]["validate"].append("erp.user_management.hooks.validate_user_permissions")
