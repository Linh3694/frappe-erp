...
def get_period_registrations(
    ...
    try:
        # Lấy params từ request - ưu tiên form_dict/args để đảm bảo nhận đúng từ client
        def _get_param(key, default):
            val = frappe.form_dict.get(key) or (frappe.request.args.get(key) if frappe.request else None)
            return val if val is not None else default

        period_id = period_id or _get_param('period_id', None)
        ...
        if incomplete_only_flag:
            ...
            if not registration_dates or not active_school_year_id or not education_stage_ids:
                return success_response(
                    data={
                        "items": [],
                        "total": 0,
            ...
            fully_sub = f"""
                SELECT t.student_id FROM (
                    ...
                    AND r.student_id IN (
                        ...
                        INNER JOIN `tabSIS Class` c2 ON cs2.class_id = c2.name
                        INNER JOIN `tabSIS Education Grade` eg2 ON c2.education_grade = eg2.name
                        WHERE {eligible_where_inner}
                    )
                    GROUP BY r.student_id
                    HAVING COUNT(DISTINCT DATE(ri.date)) = %s
                ) t
            """
            fully_params = (period_id,) + tuple(registration_dates) + eligible_params + (n_days,)
            ...
            count_sql = f"""
                ...
                FROM (
                    ...
                    AND cs.student_id NOT IN ({fully_sub})
                    GROUP BY cs.student_id
                ) inc
                INNER JOIN `tabCRM Student` s ON s.name = inc.student_id
                WHERE 1=1 {search_clause}
            """
            count_params = eligible_params + fully_params + search_params
...
def send_menu_registration_reminder():
    ...
    try:
        ...
        # Wave 2 - G.4: Menu registration reminder — N có thể > 200 → enqueue async
        try:
