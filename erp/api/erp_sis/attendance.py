...
def _cell_attendance_lookup_key(date, period, class_id):
	"""Khóa ô TKB — khớp buildLessonCellKey trên FE."""
	return f"{date}|{period}|{class_id}"


def _batch_attendance_has_for_cell_items(items):
	"""
	Đã điểm danh thủ công (present/absent/late) theo ô TKB.
	Trả về dict cell_key -> { has_attendance: bool }.
	"""
	if not items:
		return {}
	...
	for raw in items:
		...
		if cell_key in seen:
			continue
		seen.add(cell_key)
		normalized.append({"class_id": class_id, "date": date, "period": period, "cell_key": cell_key})

	if not normalized:
		return {}

	class_ids = list({i["class_id"] for i in normalized})
	dates = list({i["date"] for i in normalized})

	rows = frappe.get_all(
		"SIS Class Attendance",
		filters={"class_id": ["in", class_ids], "date": ["in", dates]},
		fields=["class_id", "date", "period", "status"],
	)

	result = {i["cell_key"]: {"has_attendance": False} for i in normalized}
	for item in normalized:
		...
		for row in rows or []:
			...
			if str(row.get("date") or "") != str(date):
				continue
			if not _period_name_matches(period, row.get("period")):
				continue
			if row.get("status") in ("present", "absent", "late"):
				result[cell_key] = {"has_attendance": True}
				break
...
def batch_get_cell_attendance_flags():
	"""
	Trạng thái điểm danh nhẹ cho lưới TKB (không trả danh sách học sinh).

	POST body: { "items": [{ "class_id", "date", "period" }, ...] }
	Returns: { "YYYY-MM-DD|Tiết 1|CLASS-001": { "has_attendance": true }, ... }
	"""
	import hashlib
	import time
	...
	try:
		...
		try:
			...
			if cached_data is not None:
				return success_response(data=cached_data, message="Cell attendance flags (cached)")
		except Exception as cache_error:
			frappe.logger().warning(f"Cache read failed: {cache_error}")

		start = time.time()
		result = _batch_attendance_has_for_cell_items(items)
		elapsed = (time.time() - start) * 1000
		frappe.logger().info(f"batch_get_cell_attendance_flags: {len(result)} keys in {elapsed:.0f}ms")
		...
		return success_response(data=result, message="Cell attendance flags fetched successfully")
	except Exception as e:
		frappe.log_error(f"batch_get_cell_attendance_flags error: {str(e)}")
		return error_response(
			message="Failed to fetch cell attendance flags",
			code="CELL_ATTENDANCE_FLAGS_ERROR",
		)
