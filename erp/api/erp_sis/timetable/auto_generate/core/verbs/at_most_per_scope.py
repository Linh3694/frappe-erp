			...
			for c in inp.classes:
				for ts_id in inp.class_subjects.get(c.name, []):
					...
					for day in inp.working_days:
						...
						le_limit(ctx, day_vars, max_d, kind=kind, weight=weight, relaxable=True, tag=f"asg_{c.name}_{ts_id}_{day}", scope={"class_id": c.name, "subject_id": ts_id, "day": day})
				...
				for inst in inst_list:
					...
					for c in inp.classes:
						...
						for day in inp.working_days:
							...
							le_limit(ctx, day_vars, max_d, kind=kind, weight=weight, relaxable=True, tag=f"sub_{ts_id}_{c.name}_{day}", scope={"subject_id": ts_id, "class_id": c.name, "day": day})
			else:
						...
						for day in inp.working_days:
							...
							for (c_id, ts_id) in tcs.get(tid, []):
								for p_idx in range(ctx.num_periods):
									...
									if v is not None:
										day_vars.append(v)
							le_limit(ctx, day_vars, limit, kind=kind, weight=weight, relaxable=True, tag=f"tch_{tid}_{day}", scope={"teacher_id": tid, "day": day})
					else:
						week_vars = []
						for (c_id, ts_id) in tcs.get(tid, []):
							for day in inp.working_days:
								for p_idx in range(ctx.num_periods):
									...
									if v is not None:
										week_vars.append(v)
						le_limit(ctx, week_vars, limit, kind=kind, weight=weight, relaxable=True, tag=f"tch_{tid}_week", scope={"teacher_id": tid})
			else:
