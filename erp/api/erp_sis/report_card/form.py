...
CRUD APIs cho Report Card Form (mẫu biểu).
"""
...
def _doc_to_dict(doc):
    """Chuyển đổi Form document thành dict cho API response."""
    pages = []
    try:
...
def get_all_forms(page: int = 1, limit: int = 50, include_all_campuses: int = 0):
    """Lấy danh sách forms với pagination."""
    try:
        page = int(page or 1)
        ...
        rows = frappe.get_all(
            ...
            fields=[
                "name", "code", "title", "program_type",
                "scores_enabled", "homeroom_enabled", "subject_eval_enabled",
...
@frappe.whitelist(allow_guest=False)
def get_form_by_id(form_id: Optional[str] = None):
    """Lấy chi tiết một form theo ID."""
    try:
        form_id = form_id or (frappe.local.form_dict or {}).get("form_id")
        if not form_id and getattr(frappe, "request", None) and getattr(frappe.request, "args", None):
            form_id = frappe.request.args.get("form_id")
        if not form_id:
        ...
        if doc.campus_id != get_current_campus_id():
...
def create_form():
    ...
    try:
        ...
        exists = frappe.db.exists(
            "SIS Report Card Form", 
            {"code": (data.get("code") or "").strip(), "campus_id": campus_id}
        )
        if exists:
            return validation_error_response(
...
@frappe.whitelist(allow_guest=False, methods=["POST"])
def update_form(form_id: Optional[str] = None):
    """Cập nhật một form."""
    try:
        data = get_request_payload()
...
@frappe.whitelist(allow_guest=False, methods=["POST"])
def delete_form(form_id: Optional[str] = None):
    """Xóa một form."""
    try:
        form_id = form_id or (frappe.local.form_dict or {}).get("form_id") or (get_request_payload().get("form_id"))
        if not form_id:
            return validation_error_response(
                message="Form ID is required", 
                errors={"form_id": ["Required"]}
...
def ensure_default_forms():
    """
    Đảm bảo các form mặc định VN tồn tại cho campus hiện tại.
...
def ensure_intl_forms():
    """
    Đảm bảo các form mặc định INTL tồn tại cho campus hiện tại.
    
    Codes:
    - PRIM_INTL: Tiểu học
    - SEC_INTL: Trung học Cơ sở
    - HIGH_INTL: Trung học Phổ thông
    - HIGH_INTL_AP: Trung học Phổ thông - AP
    """
    try:
        campus_id = get_current_campus_id()
        defaults = [
