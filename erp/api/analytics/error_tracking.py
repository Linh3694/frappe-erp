...
def get_slow_apis(limit=50, since_minutes=60, min_response_time=1000):
    ...
    try:
        ...
        return {
            ...
            "total": 0,
            "since_minutes": since_minutes,
            "min_response_time_applied": min_response_time,
            "message": (
                "Slow API không còn lưu DB. Tra cứu Grafana (WIS → slow-api) "
                "hoặc LogQL: | json | event_kind=\"slow_api\"."
            ),
        }
...
def get_slow_api_stats(period="24h"):
    ...
    try:
        return {
            ...
            "period": period,
            "message": "Thống kê slow API dùng Grafana/Loki thay vì database.",
        }
