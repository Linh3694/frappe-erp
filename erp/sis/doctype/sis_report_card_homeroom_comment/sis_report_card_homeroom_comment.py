...
class SISReportCardHomeroomComment(Document):
	def validate(self):
		# Validate that title is not empty
		if not self.title or not self.title.strip():
			frappe.throw(_("Nhận xét chủ nhiệm không được để trống"))

		# Validate that at least one option exists
		if not self.options or len(self.options) == 0:
			frappe.throw(_("Phải có ít nhất một tùy chọn nhận xét"))
	...
	def autoname(self):
		...
		if self.title and self.campus_id:
			...
			# Ensure unique name
			while frappe.db.exists("SIS Report Card Homeroom Comment", name):
