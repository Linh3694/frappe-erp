...
class SISFinanceOrderStudent(Document):
    ...
    def calculate_outstanding(self):
        ...
        if self.payment_scheme_choice == 'semester':
            sem1 = flt(self.semester_1_amount or 0)
            sem2 = flt(self.semester_2_amount or 0)
            total_semester = sem1 + sem2
            paid_sem = flt(self.semester_1_paid or 0) + flt(self.semester_2_paid or 0)
            # Ưu tiên tổng đã đóng theo từng kỳ; nếu chưa nhập kỳ thì giữ paid_amount tổng
            # (import Excel chỉ có một cột paid_amount, không ghi semester_*_paid → trước đây bị ghi đè 0).
            if paid_sem > 0:
                paid = paid_sem
            else:
                paid = flt(self.paid_amount or 0)
            self.total_amount = total_semester
            self.paid_amount = paid
            self.outstanding_amount = total_semester - paid
        else:
            self.outstanding_amount = flt(self.total_amount or 0) - flt(self.paid_amount or 0)
    ...
    def update_payment_status(self):
        ...
        if self.payment_scheme_choice == 'semester':
            s1 = flt(self.semester_1_amount or 0)
            s2 = flt(self.semester_2_amount or 0)
            p1 = flt(self.semester_1_paid or 0)
            p2 = flt(self.semester_2_paid or 0)
            # Có paid_amount tổng (vd. import Excel) nhưng chưa tách kỳ trong DB → phân bổ ảo để xác định trạng thái
            if p1 == 0 and p2 == 0 and flt(self.paid_amount or 0) > 0 and (s1 > 0 or s2 > 0):
                rem = flt(self.paid_amount or 0)
                if s1 > 0:
                    p1 = min(rem, s1)
                    rem -= p1
                if s2 > 0:
